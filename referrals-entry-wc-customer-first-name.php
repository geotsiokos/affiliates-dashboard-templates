	...
	<div class="cell <?php echo ( $index === 0 ? 'first' : '' ) . ' ' . ( ( $index + 1 ) % 2 === 0 ? 'even' : 'odd' ); ?> <?php echo esc_attr( $key ); ?>" data-heading="<?php echo esc_attr( $column['title'] ); ?>">
		...
		switch ( $key ) {
			...
			case 'status' :
				...
				break;
			case 'items' :
				if ( isset( $entry->data ) && strlen( $entry->data ) > 0 ) :
					$unserialized_data = unserialize( $entry->data ); //error_log( print_r( $unserialized_data, true ) );
					if ( isset( $unserialized_data['order_id'] ) && is_array( $unserialized_data['order_id'] ) ) :
						if ( isset( $unserialized_data['order_id']['value'] ) ) :
							$order_id = $unserialized_data['order_id']['value'];
							$referred_order = new WC_Order( $order_id); ?>
							<div class="referral-items">
								<?php echo esc_html( $referred_order->get_billing_first_name() ); ?>
							</div>
						<?php endif;
					endif;
				endif;
