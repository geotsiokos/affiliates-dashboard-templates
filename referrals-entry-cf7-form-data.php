	...
	<div class="cell <?php echo ( $index === 0 ? 'first' : '' ) . ' ' . ( ( $index + 1 ) % 2 === 0 ? 'even' : 'odd' ); ?> <?php echo esc_attr( $key ); ?>" data-heading="<?php echo esc_attr( $column['title'] ); ?>">
		...
		switch ( $key ) {
			...
			case 'status' :
				...
				break;
			case 'items' :
				if ( isset( $entry->integration ) && $entry->integration == 'affiliates-contact-form-7' ) : ?>
					<?php if ( isset( $entry->data ) && strlen( $entry->data ) > 0 ) : 
						$unserialized_data = unserialize( $entry->data ); 
						if ( isset( $unserialized_data ) && is_array( $unserialized_data ) ) :
							if ( isset( $unserialized_data['your-name'] ) && isset( $unserialized_data['your-name']['value'] ) ) : ?>
								<div class="referral-items">
									<div class="referral-item">
										<div class="referral-item-title"><?php echo esc_html( $unserialized_data['your-name']['value'] ); ?></div>
									</div>
								</div>
							<?php else : ?>
