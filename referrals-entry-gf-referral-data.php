	...
	<div class="cell <?php echo ( $index === 0 ? 'first' : '' ) . ' ' . ( ( $index + 1 ) % 2 === 0 ? 'even' : 'odd' ); ?> <?php echo esc_attr( $key ); ?>" data-heading="<?php echo esc_attr( $column['title'] ); ?>">
		...
		switch ( $key ) {
			...
			case 'status' :
				...
				break;
			case 'items' :
				if ( isset( $entry->integration ) && $entry->integration == 'affiliates-gravityforms' ) : ?>
					<?php if ( isset( $entry->data ) && strlen( $entry->data ) > 0 ) : 
						$unserialized_data = unserialize( $entry->data ); 
						if ( $unserialized_data && is_array( $unserialized_data ) ) : ?>
							<div class="referral-items">
								<?php foreach ( $unserialized_data as $data => $value ) : ?>
									<div class="referral-item"><?php error_log( print_r( $value, true ) ); ?>
										<?php if ( $value['title'] !== 'Entry' ) : ?>
											<div class="referral-item-title"><?php echo esc_html( $value['title'] ); ?></div>
											<div class="referral-item-amount"><?php echo stripslashes( wp_filter_kses( $value['value'] ) ); ?></div>
										<?php endif; ?>
									</div>
								<?php endforeach; ?>
							</div>
