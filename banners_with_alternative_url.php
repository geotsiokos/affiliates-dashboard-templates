...
$alternative_banner_url = 'http://dev.xt/shop/';
?>
<h2><?php esc_html_e( 'Banners', 'affiliates' ); ?></h2>
<div class="dashboard-section dashboard-section-banners">
	<form id="dashboard-banners-search" class="filters capsule-container" action="" method="post" >
		<div class="capsule full">
			<label for="banner-url"><?php esc_html_e( 'Link banners to &hellip;', 'affiliates' ); ?></label>
		...
		<div class="capsule full">
			<input type="text" placeholder="<?php esc_attr_e( 'Search &hellip;', 'affiliates' ); ?>" value="<?php echo esc_attr( stripslashes( $section->get_search() ) ); ?>" name="search">
		</div>
		...
		foreach ( $banners as $banner ) : ?>
			<div class="banners-container">
				<div class="cell banner-image">
					<?php
						if ( empty( $_REQUEST['banner-url'] ) ) {
							$banner->url = Affiliates_Url_Renderer_WordPress::render_affiliate_url(
								array(
									'url'  => $alternative_banner_url,
									'type' => 'append'
								)
							);
						}
