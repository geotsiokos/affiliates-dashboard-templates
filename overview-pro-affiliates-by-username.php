...
<div class="dashboard-section dashboard-section-overview">
	<div class="stats-container">
		<div class="stats-item">
			<div class="stats-item-heading"><?php _e( 'Visits', 'affiliates' ); ?></div>
			<div class="stats-item-value"><?php echo esc_html( $visits ); ?></div>
		</div>
		<div class="stats-item">
			<div class="stats-item-heading"><?php _e( 'Referrals', 'affiliates' ); ?></div>
			<div class="stats-item-value"><?php echo esc_html( $referrals ); ?></div>
		</div>
		<div class="stats-item">
			<div class="stats-item-heading"><?php _e( 'Earnings', 'affiliates' ); ?></div>
	...
	<div class="affiliates-dashboard-overview-filters">
		<form class="filters" method="post" action="">
			<div class="capsule-container">
				<div class="capsule">
					<select name="range">
						<option <?php echo ( $selected === 'last-seven-days' ? ' selected="selected" ' : '' ); ?> value="last-seven-days"><?php esc_html_e( 'Last 7 Days', 'affiliates' ); ?></option>
						<option <?php echo ( $selected === 'last-thirty-days' ? ' selected="selected" ' : '' ); ?> value="last-thirty-days"><?php esc_html_e( 'Last 30 Days', 'affiliates' ); ?></option>
						<option <?php echo ( $selected === 'last-ninety-days' ? ' selected="selected" ' : '' ); ?> value="last-ninety-days"><?php esc_html_e( 'Last 90 Days', 'affiliates' ); ?></option>
						<option <?php echo ( $selected === 'weekly' ? ' selected="selected" ' : '' ); ?> value="weekly"><?php esc_html_e( 'Weekly', 'affiliates' ); ?></option>
						<option <?php echo ( $selected === 'monthly' ? ' selected="selected" ' : '' ); ?> value="monthly"><?php esc_html_e( 'Monthly', 'affiliates' ); ?></option>
						<option <?php echo ( $selected === 'yearly' ? 'selected="selected" ' : '' ); ?> value="yearly"><?php esc_attr_e( 'Yearly', 'affiliates' ); ?></option>
						<option <?php echo ( $selected === 'custom' ? 'selected="selected" ' : '' ); ?> value="custom"><?php esc_attr_e( 'Custom Date Range', 'affiliates' ); ?></option>
					</select>
				</div>
				<div class="capsule">
					<div class="capsule-atom">
						<label class="from-date-filter" for="from_date"><?php esc_html_e( 'From', 'affiliates' ); ?></label>
						<input class="datefield from-date-filter" name="from-date" type="date" value="<?php echo !empty( $from_date ) ? esc_attr( $from_date ) : ''; ?>" >
					</div>
					<div class="capsule-atom">
						<label class="thru-date-filter" for="thru_date"><?php esc_html_e( 'Until', 'affiliates' ); ?></label>
						<input class="datefield thru-date-filter" name="thru-date" type="date" value="<?php echo !empty( $thru_date ) ? esc_attr( $thru_date ) : ''; ?>" >
					</div>
				</div>
			</div>
			<div class="filter-buttons">
				<input type="submit" name="apply_filters" value="<?php esc_attr_e( 'Apply', 'affiliates' ); ?>" >
				<input type="submit" name="clear_filters" value="<?php esc_attr_e( 'Clear', 'affiliates' ); ?>" >
			</div>
		</form>
	</div>
	<?php
	...
	<div class="affiliates-dashboard-overview-link link-generator capsule-container">
		...
		<?php 
			$affiliate_id = affiliates_encode_affiliate_id( $section->get_affiliate_id() );
			if ( class_exists( 'Affiliates_By_Username' ) ) { 
		?>
			<div class="capsule full">
				<label>
					<textarea id="copy-to-clipboard-source" class="affiliate-url" readonly="readonly" onmouseover="this.focus();" onfocus="this.select(); affiliates_by_username(affiliates_overview_controls);"><?php echo do_shortcode( '[affiliates_by_username][affiliates_url][/affiliates_by_username]' ); ?></textarea>
		...
		?>
			<div class="capsule full">
				<label>
					<textarea id="copy-to-clipboard-source" class="affiliate-url" readonly="readonly" onmouseover="this.focus();" onfocus="this.select(); affiliates_by_username(affiliates_overview_controls);"><?php echo esc_html( Affiliates_Shortcodes::affiliates_url( array() ) ); ?></textarea>
					<?php esc_html_e( 'Your affiliate URL', 'affiliates' ); ?></p>
				</label>
			</div>
		<?php } ?>
		<script type="text/javascript">
			function affiliates_by_username(obj) {
				if ( typeof obj != "undefined" ) {
					if ( obj.hasOwnProperty( 'affiliate_id' ) ) {
						obj.affiliate_id = '<?php echo $affiliate_id; ?>';
					}
				}
			}
		</script>
